...
type StoredAuthUser = { role?: string; email?: string };
...
const isRoleOrEmailAdmin = (role?: string, email?: string | null) => {
  const normalizedRole = role?.toLowerCase().trim();
  if (normalizedRole === 'admin' || normalizedRole === 'superadmin') return true;
  const normalizedEmail = email?.toLowerCase().trim();
  if (normalizedEmail && DEV_EMAILS.includes(normalizedEmail)) return true;
  return false;
};
...
export default function CustomOrder({ addToCart, user }: CustomOrderProps) {
  ...
  const isAdmin = useCallback(() => {
    if (isRoleOrEmailAdmin(user?.role, user?.email)) return true;
    ...
    // Any active admin panel session
    if (adminToken) return true;
    if (adminUser && isRoleOrEmailAdmin(adminUser?.role, adminUser?.email)) return true;
    if (storefrontToken && isRoleOrEmailAdmin(storefrontUser?.role, storefrontUser?.email)) return true;
  ...
  const getAdminAuthToken = useCallback(() => {
    ...
    if (storefrontToken && isRoleOrEmailAdmin(storefrontUser?.role, storefrontUser?.email)) return storefrontToken;
    if (storefrontToken && isRoleOrEmailAdmin(user?.role, user?.email)) return storefrontToken;
                ...
                {isAdmin() && (
                  <div className="space-y-1.5 pt-1">
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      <button onClick={handleDownloadPreview} disabled={!imageState}
                        className="h-7 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-bold flex items-center justify-center gap-1 hover:bg-emerald-100 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                        title={!imageState ? 'Upload image first' : 'Download Mockup'}>
                        <Download className="w-2.5 h-2.5" /> Mockup
                      </button>
                      <button onClick={handleDownloadTemplate} disabled={!imageState}
                        className="h-7 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200 text-[10px] font-bold flex items-center justify-center gap-1 hover:bg-indigo-100 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                        title={!imageState ? 'Upload image first' : 'Download Template'}>
                        <Download className="w-2.5 h-2.5" /> Template
                      </button>
                      <button onClick={handleDownloadInnerTemplate} disabled={!imageState}
                        className="h-7 rounded-md bg-purple-50 text-purple-700 border border-purple-200 text-[10px] font-bold flex items-center justify-center gap-1 hover:bg-purple-100 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                        title={!imageState ? 'Upload image first' : 'Download Inner Circle'}>
              ...
              {isAdmin() && (
                <div className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-4">
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                    <Download className="w-3.5 h-3.5 text-yellow-600" /> Admin Downloads
                  </h4>
                  <button onClick={handleDownloadPrintFile} disabled={downloading || !imageState}
                    className="w-full h-9 rounded-lg border border-slate-200 bg-white text-slate-700 font-bold text-xs flex items-center justify-center gap-1.5 hover:bg-slate-100 transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-sm">
                    {downloading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Download className="w-3 h-3" />} {downloading ? 'Generating...' : 'Download Print File'}
                  </button>
                  <div className="grid grid-cols-3 gap-1.5">
                    <button onClick={handleDownloadPreview} disabled={!imageState}
                      className="h-8 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200 text-[11px] font-bold flex items-center justify-center gap-1 hover:bg-emerald-100 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                      title={!imageState ? 'Upload image first' : 'Download Mockup'}>
                      <Download className="w-3 h-3" /> Mockup
                    </button>
                    <button onClick={handleDownloadTemplate} disabled={!imageState}
                      className="h-8 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200 text-[11px] font-bold flex items-center justify-center gap-1 hover:bg-indigo-100 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                      title={!imageState ? 'Upload image first' : 'Download Template'}>
                      <Download className="w-3 h-3" /> Template
                    </button>
                    <button onClick={handleDownloadInnerTemplate} disabled={!imageState}
                      className="h-8 rounded-md bg-purple-50 text-purple-700 border border-purple-200 text-[11px] font-bold flex items-center justify-center gap-1 hover:bg-purple-100 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                      title={!imageState ? 'Upload image first' : 'Download Inner Circle'}>
                      <Download className="w-3 h-3" /> Inner
                    </button>
                  </div>
                </div>
              )}
