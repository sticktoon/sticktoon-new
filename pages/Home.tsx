...
import { BADGES, CATEGORIES, HERO_BADGES, formatPrice, fetchBackendCategories, CategoryItem } from '../constants.tsx';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Sort dynamically by creation date descending (newest first)
        const sorted = list
          .filter((p: any) => p.isActive !== false)
          .sort((a: any, b: any) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());

        const mapped: FeaturedBadge[] = sorted.map((p) => ({
          id: p._id,
          name: p.name,
          price: p.price,
          category: p.category,
              ...
              <Link to={`/badge/${badge.id}`} className="w-full">
                <div className="relative w-full aspect-square rounded-xl sm:rounded-2xl bg-white flex items-center justify-center mb-3 overflow-hidden border-[3px] sm:border-[4px] border-slate-900/70 shadow-inner">
                  ...
                  <img
                    ...
                    loading="lazy"
                    decoding="async"
                    className={`relative w-[110%] h-[110%] sm:w-[120%] sm:h-[120%] object-contain drop-shadow-[0_25px_45px_rgba(0,0,0,0.28)] transition-all duration-500 group-hover:scale-[1.06] ${loadedImages.has(badge.id) ? 'opacity-100' : 'opacity-0'
                      }`}
              ...
              <div className="mt-auto">
                <div className="flex flex-row items-center gap-2">
                  <button
                    ...
                    className={`flex-1 py-2 sm:py-2.5 rounded-md font-black transition-all duration-300 flex items-center justify-center gap-1 ${badge.stock === 0
