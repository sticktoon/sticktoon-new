...
async function checkDB() {
  try {
    ...
    const User = mongoose.model("User", new mongoose.Schema({ email: String, name: String, role: String }));
    const Order = mongoose.model("Order", new mongoose.Schema({ userId: mongoose.Schema.Types.ObjectId, status: String, amount: Number, createdAt: Date }));
    const UserOrders = mongoose.model("User_Orders", new mongoose.Schema({ userId: mongoose.Schema.Types.ObjectId, orderId: mongoose.Schema.Types.ObjectId }));
